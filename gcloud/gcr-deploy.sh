...
# # apply tag
# log "${GR}Creating container named ${PP}${CONTAINER} ${GR} with tag ${PP}${target}${NC}" 2>&1
...
# # save retval of pipe
# TEST="$( gcloud builds submit --tag  "$target" 2>&1; printf :%s "${PIPESTATUS[*]}" )"
...
log "${PP}Deploy $target to gcr ${GCR_FULL}${NC}"

cmd="gcloud run deploy $CONTAINER \
   --platform managed \
  --image $target \
  --cpu $CPU \
  --memory $MEMORY \
